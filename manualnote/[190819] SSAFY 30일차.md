...
앱 폴더의 models.py 에 클래스를 선언하여 저장할 DB 형식을 정의할수 있음
...
DB 지우기 : db.sqlite3 파일 삭제 후 다시 마이그레이션? 하면 된다고 함. (오류나서 못따라갔음)
```
...
```
from articles.models import Article
Article.objects.all()  쿼리셋 출력.
article=Article()
article.title = '내용'
article.text = '내용'
article.save()
Article.objects.all()  --> 이상한거나옴.   --> models.py에 `__str__ ` 정의하면... 나옴
```



`from .models import Article` 현재와 동등레벨의 models 폴더에서 Article만 가져옴.

(중간에 따라간다고 필기 누락...... 코드 보고 정리 필요)
...
-->  main?app? urls.py에 별명 만들기.. name='별명'
```
...
from .models import Article     :models에서 정의한 Article을 띄우겠다!
